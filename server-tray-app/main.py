...
def list_chrome_profiles():
    ...
    try:
        ...
        for item in os.listdir(user_data_path):
            ...
            # Check for Default or Profile X folders
            if os.path.isdir(item_path) and (
                item == "Default" or item.startswith("Profile")
            ):
                # Try to get profile name from Local State
                    ...
                    try:
                        ...
                        with open(local_state_path, "r", encoding="utf-8") as f:
                            ...
                            if (
                                ...
                                in local_state
                                and "info_cache"
                                in local_state["profile"]
                            ):
                                profile_info = local_state["profile"]["info_cache"].get(
                                    item, {}
                                )
                                profile_name = profile_info.get("name", item)
                    except Exception:
                        pass

                profiles.append({"id": item, "name": profile_name})

        return sorted(profiles, key=lambda x: x["name"])
    except Exception as e:
        print(f"[CHROME] Error listing profiles: {e}")
        return []

def get_chrome_cookies(profile_id="Default"):
    """Extract cookies from Chrome Extension Cookies database."""
    try:
        user_data_path = get_chrome_user_data_path()
        profile_path = os.path.join(user_data_path, profile_id)
        
        print(f"[CHROME] Attempting to extract cookies from profile: {profile_id}")
        print(f"[CHROME] Profile path: {profile_path}")

        # Kill Chrome to unlock database
        print("[CHROME] Killing Chrome processes...")
        try:
            os.system(
                'taskkill /F /IM chrome.exe 2>nul || taskkill /F /IM GoogleChromePortable.exe 2>nul'
            )
            time.sleep(1)
        except Exception as e:
            print(f"[CHROME] Warning killing Chrome: {e}")

        # Try browser-cookie3 first (handles encryption automatically)
        # Import locally to avoid module-not-found at server start
        try:
            import browser_cookie3
            print("[CHROME] Trying browser-cookie3 (encrypted cookies)...")
            ...
            if cookies_dict:
                print(f"[CHROME] ✓ Extracted {len(cookies_dict)} cookies via browser-cookie3")
                return cookies_dict
            else:
                print("[CHROME] browser-cookie3 returned empty cookie jar")
        except ModuleNotFoundError as mf:
            print(f"[CHROME] browser-cookie3 import failed: {mf}")
        except Exception as bc_err:
            print(f"[CHROME] browser-cookie3 extraction failed: {type(bc_err).__name__}: {bc_err}")

        # Fallback: Try direct SQLite read from Extension Cookies
        print("[CHROME] Trying direct SQLite (Extension Cookies database)...")
        ext_cookies_path = os.path.join(profile_path, "Extension Cookies")
        
        if not os.path.exists(ext_cookies_path):
            print(f"[CHROME] Extension Cookies database not found at {ext_cookies_path}")
            return {}

        # Copy to temp to avoid locking
        temp_db = os.path.join(
            os.path.expanduser("~"), f".nexus_cookies_{int(time.time() * 1000)}"
        )
        ...
        try:
            ...
            cursor = conn.cursor()
            # Query all cookies, no filters - Chrome stores them with various value types
            ...
            print(f"[CHROME] SQLite query returned {row_count} rows, extracted {len(cookies_dict)} usable cookies")
            
            if cookies_dict:
                print(f"[CHROME] ✓ Extracted {len(cookies_dict)} cookies via SQLite")
                return cookies_dict
            else:
                print("[CHROME] No cookies with values found in Extension Cookies database")
                print("[CHROME] This means Chrome profile has no saved cookies yet")
                return {}
        finally:
            try:
                os.remove(temp_db)
        ...
        print(f"[CHROME] Unexpected error: {type(e).__name__}: {e}")
        import traceback
        traceback.print_exc()
        return {}


def generate_session_injection_code(cookies_dict, profile_name="Default"):
    """Generate JavaScript to inject session data into website.
    
    Since localStorage/sessionStorage are per-origin (CORS restricted),
    this returns code that user runs INSIDE the website they need to access.
    This injects:
    1. Any extracted cookies
    2. localStorage data
    3. Instruction to also check sessionStorage in DevTools
    """
    
    js_code = f"""
// ═══════════════════════════════════════════════════════════════
// NEXUS SESSION INJECTOR - Extracted from {profile_name} profile
// Run this in Console (F12) on the website you need to access
// ═══════════════════════════════════════════════════════════════

(function() {{
    console.log('%c🔐 NEXUS Session Injector Starting...', 'color: blue; font-weight: bold;');
    
    // ─────── INJECT COOKIES ───────
    const cookies = {json.dumps(cookies_dict)};
    let cookieCount = 0;
    ...
    for (const [name, value] of Object.entries(cookies)) {{
        try {{
            ...
            console.warn(`Failed to set cookie ${{name}}: ${{e.message}}`);
        }}
    }}
    
    console.log(`%c✓ Injected ${{cookieCount}} cookies`, 'color: green; font-weight: bold;');
    
    // ─────── PRESERVE SESSION DATA ───────
    // Since localStorage is isolated per-origin, this code runs INSIDE the website.
    // Check if you can access sessionStorage for additional session data:
    
    if (sessionStorage.length > 0) {{
        console.log(`%c✓ Found ${{sessionStorage.length}} items in sessionStorage`, 'color: green');
        let sessionData = {{}};
    ...
    if (localStorage.length > 0) {{
        console.log(`%c✓ Found ${{localStorage.length}} items in localStorage`, 'color: green');
        let localData = {{}};
    ...
    // ─────── REFRESH PAGE ───────
    console.log('%c⏳ Refreshing page to apply session...', 'color: blue');
    setTimeout(() => {{
...
# ── Chrome Cookies Endpoints ──
@app.get("/cookies/profiles", dependencies=[Depends(verify_pin)])
def get_cookies_profiles():
    ...
    try:
        ...
        print(f"[COOKIES] Error listing profiles: {e}")
        return {"error": str(e)}


@app.get("/cookies/get", dependencies=[Depends(verify_pin)])
def get_cookies_from_profile(profile: str = "Default"):
    """Extract cookies from specified Chrome profile and format for console injection."""
    try:
        cookies = get_chrome_cookies(profile)
        if not cookies:
            # No cookies found - this is the issue
            return {
                "error": "No cookies in this profile",
                "status": "no_cookies",
                "profile_name": profile,
                "detailed_message": (
                    "❌ No cookies found in Chrome profile.\n\n"
                    "IMPORTANT: Modern websites (LinkedIn, Gmail, etc.) may NOT use HTTP cookies.\n"
                    "They use localStorage/sessionStorage tokens instead.\n\n"
                    "═══════════════════════════════════════════════════════════════\n"
                    "🔧 HOW TO GET YOUR SESSION:\n"
                    "═══════════════════════════════════════════════════════════════\n\n"
                    "OPTION 1: Use DevTools (Most Reliable)\n"
                    "────────────────────────────────────\n"
                    "1. Open Chrome and log into the website you need\n"
                    "2. Press F12 to open Developer Tools\n"
                    "3. Go to 'Application' tab (or 'Storage')\n"
                    "4. Click 'Cookies' on the left → select the website\n"
                    "5. Look for cookies with these names: JSESSIONID, sessionid, auth, token\n"
                    "6. Copy their values\n"
                    "7. Also check 'LocalStorage' → find any auth tokens\n\n"
                    "OPTION 2: Let Chrome Save Cookies First\n"
                    "────────────────────────────────────────\n"
                    "1. Close Chrome completely\n"
                    "2. Open Chrome fresh\n"
                    "3. Log into the website\n"
                    "4. Visit several pages on the website while logged in\n"
                    "5. Let Chrome sit for 5 seconds\n"
                    "6. Close Chrome completely\n"
                    "7. Try extraction again\n\n"
                    "═══════════════════════════════════════════════════════════════\n"
                    f"Chrome Profile: {profile}\n"
                    f"Chrome Data Location: {get_chrome_user_data_path()}\n"
                    "═══════════════════════════════════════════════════════════════"
                ),
            }

        # We have cookies - generate injection code
        js_code = generate_session_injection_code(cookies, profile)
        ...
        return {
            ...
            "instructions": (
                "USAGE INSTRUCTIONS:\n"
                "1. Go to the lab website in Chrome\n"
                "2. Press F12 to open Developer Tools\n"
                "3. Click 'Console' tab\n"
                "4. Paste the code from 'js_code' field\n"
                "5. Press Enter\n"
                "6. Refresh the page (F5)\n"
                "7. You should be logged in!\n\n"
                f"Total cookies injected: {len(cookies)}"
            ),
        }
    except Exception as e:
        print(f"[COOKIES] Error getting cookies: {e}")
        return {
            "error": str(e),
            "status": "error",
            ...
            "advice": (
                ...
                "3. Search for 'chrome.exe'\n"
                "4. If found, right-click → End Task\n"
                "5. Wait 2 seconds\n"
                "6. Try extracting again\n\n"
                "If still fails, use OPTION 1 (DevTools method) above."
            ),
        }


@app.get("/cookies/extract-live", dependencies=[Depends(verify_pin)])
def extract_cookies_live():
    """Extract cookies by closing Chrome gracefully and reading database.
    
    This is better than /cookies/get because it:
    1. Closes Chrome gracefully (allows cookie flush to disk)
    2. Reads Extension Cookies database directly
    3. Returns injectable JavaScript code
    """
    try:
        print("[COOKIES] Starting live extraction (will close Chrome)...")
        
        # Step 1: Close Chrome gracefully
        print("[COOKIES] Step 1: Closing Chrome...")
        try:
            subprocess.run(
        ...
        time.sleep(2)  # Wait for cookie flush
        
        # Step 2: Force kill if still running
        result = subprocess.run(['tasklist'], capture_output=True, text=True)
        if 'chrome.exe' in result.stdout:
            print("[COOKIES] Chrome still running, force killing...")
            os.system('taskkill /F /IM chrome.exe 2>nul')
            time.sleep(1)
        
        # Step 3: Extract cookies
        print("[COOKIES] Step 2: Extracting from Extension Cookies database...")
        chrome_data = os.path.expanduser(r"~\AppData\Local\Google\Chrome\User Data")
        profile_path = os.path.join(chrome_data, "Default")
        ext_cookies_path = os.path.join(profile_path, "Extension Cookies")
        
        if not os.path.exists(ext_cookies_path):
            return {
                "status": "error",
                "error": "Extension Cookies file not found",
                "advice": f"Expected location: {ext_cookies_path}"
            }
        
        # Copy and read
        temp_db = os.path.join(os.path.expanduser("~"), f".nexus_extract_{int(time.time() * 1000)}")
        cookies_dict = {}
        ...
        try:
            ...
            conn.close()
            print(f"[COOKIES] ✓ Extracted {len(cookies_dict)} cookies")
        ...
        if not cookies_dict:
            return {
                "status": "no_cookies",
                "error": "No cookies in database after closing Chrome",
                "advice": (
                    "Chrome may need to save cookies first.\n"
                    "Try: Open Chrome → Log in → Wait 5 sec → Close → Try again"
                )
            }
        
        # Step 4: Generate injection code
        js_code = generate_session_injection_code(cookies_dict, "Default")
        ...
        return {
            ...
            "js_code": js_code,
            "instructions": (
                "✅ SUCCESS! Here's what to do:\n\n"
                "1. Go to the lab website in Chrome (it will now be closed, open it)\n"
                "2. Log in again (or open if you're still logged in)\n"
                "3. Press F12 to open Developer Tools\n"
                "4. Click 'Console' tab\n"
                "5. Copy the 'js_code' value from this response\n"
                "6. Paste it into the Console\n"
                "7. Press Enter\n"
                "8. Refresh the page (F5)\n"
                "9. You should now be auto-logged in! 🎉"
            )
        }
        
    except Exception as e:
        print(f"[COOKIES] Error in live extraction: {type(e).__name__}: {e}")
        import traceback
        traceback.print_exc()
        return {
            "status": "error",
